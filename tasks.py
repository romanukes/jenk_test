...
NAME = os.getenv("IMAGE_NAME", f"helloworld-flask-py{PYTHON_VER}")
# Tag for the image
...
def build_image(context, name=NAME, python_ver=PYTHON_VER, image_ver=IMAGE_VER):
    ...
    result = context.run(
        f"docker build --tag {name}:{image_ver} --build-arg PYTHON_VER={python_ver} -f Dockerfile .", hide=True
    )
    if result.exited != 0:
